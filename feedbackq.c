...
Queue_t *get_queue_by_id(int queue_id) {
	...
	switch (queue_id) {            //writing the condtions for the cases 
		case 1:
			return queue_1;
...
void handle_instruction(Instruction_t *instruction, int tick) {
	 ...
	 int task_id = instruction->task_id - 1; 

    if(instruction->burst_time == 0) {
        
        
        task_table[task_id].id = instruction->task_id; // New Task
        task_table[task_id].current_queue = 1;
        task_table[task_id].total_wait_time = 0;
        task_table[task_id].total_execution_time = 0;
        task_table[task_id].burst_time = 0;
        task_table[task_id].remaining_burst_time = 0;
        ...
        Task_t *task = &task_table[task_id]; // Task Termination
        
        printf("[%05d] id=%04d EXIT wt=%d tat=%d\n", tick, task_id + 1, task->total_wait_time, task->total_execution_time + task->total_wait_time);
        
        task->id = 0; // Reset the task ID 
        
    } else {
        
        
        Task_t *task = &task_table[task_id]; // CPU Burst for the task
        
        if (task->id == 0) { // initialize 
        ...
        Queue_t *queue = get_queue_by_id(task->current_queue);
        enqueue(queue, task);
...
Task_t *peek_priority_task() {
    ...
    if (!is_empty(queue_1)){
        ...
        return queue_1->start; //setting the queue
...
void boost(int tick) {
    ...
    // Move tasks from Queue 3 to Queue 1
    while (!is_empty(queue_3)) {
        
        Task_t *task = dequeue(queue_3);
        task->current_queue = 1; // queue to the highest priority
        enqueue(queue_1, task);
        
    }

    // Move tasks from Queue 2 to Queue 1
    ...
    // If a current task exists do the condition
    if (current_task != NULL && remaining_quantum > 2) {
...
void execute_task(int tick) {
	...
	if (current_task != NULL) {
        ...
        printf("[%05d] id=%04d req=%d used=%d queue=%d\n", tick, current_task->id, current_task->burst_time, current_task->burst_time - current_task->remaining_burst_time, current_task->current_queue);

        if (current_task->remaining_burst_time == 0) {
            
            current_task = NULL; // Task done
            
        } else if (remaining_quantum == 0) {
            
            
            decrease_task_level(current_task); //move to the next
...
// Increment waiting time 
    ...
    if (current_task != NULL) {
        ...
        current_task->total_execution_time++; // incrementing the time 
